...
export default {
  ...
  setup() {
    ...
    const getChangeColor = (forecast) => {
      ...
      // If change is within ±2%, consider it stable and show primary
      if (changePercent <= 2) {
        return 'var(--color-primary)' // Primary for stable
      }

      if (change > 0) return 'var(--color-accent)' // Accent for increasing
      if (change < 0) return 'var(--danger)' // Danger for decreasing
      return 'var(--color-primary)' // Primary for no change
    }
